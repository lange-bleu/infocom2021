...
for speaker_id = speaker_ids
    ...
    for ii=1:data_type.size(2)
        if ii==1
            error_dir = [data_root, speaker_id, data_type(ii),'hide_eliminated.xlsx'];
        elseif ii==2
            error_dir = [data_root, speaker_id, data_type(ii), '0dB', 'hide_eliminated.xlsx'];
        else
            error_dir = [data_root, speaker_id, 'noise', data_type(ii), '0dB', 'hide_eliminated.xlsx'];
        end
    ...
    for jj=1:4
        ...
        save_array = ['./',speaker_id, '/','eliminated-', metric(jj), '.pdf'];
