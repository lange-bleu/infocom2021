...
for speaker_id = speaker_ids
    ...
    for ii=1:data_type.size(2)
        if ii==1
            error_dir = [data_root, speaker_id, data_type(ii),'focus_expect_focused.xlsx'];
        elseif ii==2
            error_dir = [data_root, speaker_id, data_type(ii), '0dB', 'focus_expect_focused.xlsx'];
        else
            error_dir = [data_root, speaker_id, 'noise', data_type(ii), '0dB', 'focus_expect_focused.xlsx'];
        end
    ...
    for jj=1:4   % set to 1:4 for previous focus xlsx
        ...
        save_array = ['./',speaker_id, '/', metric(jj), '.pdf'];
