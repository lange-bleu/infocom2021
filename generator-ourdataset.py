...
def mix_conversation2(hp, args, audio, num, s1_dvec, s1_target, s2, spk, train):
    ...
    d, _ = librosa.effects.trim(d, top_db=60)
    w1, _ = librosa.effects.trim(w1, top_db=60)
    w2, _ = librosa.effects.trim(w2, top_db=60)
...
def mix_joint2(hp, args, audio, num, s1_dvec, s1_target, s2, spk, train):
    # Add SNR option here
    srate = hp.audio.sample_rate
    dir_ = os.path.join(args.out_dir, 'train' if train else 'test')
    speaker_id = spk[0].split('/')[-2]
    sub_dir_ = os.path.join(dir_, speaker_id, 'joint')
    os.makedirs(sub_dir_, exist_ok=True)
...
def mix_noise(hp, args, audio, num, s1_dvec, s1_target, noise_mat, noise_type, spk, train):
    srate = hp.audio.sample_rate

    dir_ = os.path.join(args.out_dir, 'train' if train else 'test')
    speaker_id = spk[0].split('/')[-2]
    sub_dir_ = os.path.join(dir_, speaker_id, noise_type)
    os.makedirs(sub_dir_, exist_ok=True)
    d, _ = librosa.load(s1_dvec, sr=srate)
    w1, _ = librosa.load(s1_target, sr=srate)
    ...
    mixed = Add_noise(signal=w1, noise=noise, SNR=0)
...
if __name__ == '__main__':
    ...
    if args.libri_dir is not None:
        ...
        #    if os.path.isdir(x)]
        test_folders = [x for x in glob.glob(os.path.join(args.libri_dir, 'audio', '*'))]
    ...
    def test_wrapper(num):
        ...
        mix_joint2(hp, args, audio, num, s1_dvec, s1_target, s2, spk1, train=False)
        mix_conversation2(hp, args, audio, num, s1_dvec, s1_target, s2, spk1, train=False)
        noise_sources = glob.glob('./selected_noise/*')
        for noise_mat in noise_sources:
            noise_type = noise_mat.split('/')[-1][:-4]
            mix_noise(hp, args, audio, num, s1_dvec, s1_target, noise_mat, noise_type, spk1, train=False)
    # arr = list(range(10 ** 4))
